...
router.post("/login",async (req,res)=>{
    ...
    if (user==="matias" && password==="juan") {
        req.session.userData= {user:user ,admin:true}
...
router.get("/private",auth, (req,res)=>{
    res.send("si estas viendo esto es que estas autenticado")
})
router.get("/logout",(req,res)=>{
    req.session.destroy(err=>{
        if(!err) res.send("sesion destruida con exito")
    })
})
